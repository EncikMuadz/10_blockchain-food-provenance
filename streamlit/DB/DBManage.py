...
class DBManage:
    ...
    def load_config(self):
        ...
        self.CONFIG_LOCATION = os.path.join(BASE_DIR, 'db.sqlite3')
        if not os.path.exists(self.CONFIG_LOCATION):
            ...
            with open(self.CONFIG_LOCATION + 'config.json', 'w') as json_data_file:
                json.dump(json.loads(
                    '{ "DEFAULT": { "DATABASE_NAME": "supplychain_database.bc", "CONFIG_FILE": "config.json"}}'),
                    json_data_file)
        else:
    ...
    def load_logchain(self, logchain):
